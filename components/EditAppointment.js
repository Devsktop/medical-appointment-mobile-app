...
const EditAppointment = ({ navigation }) => {
  ...
  const [appointmentOldDate, setAppointmentOldDate] = useState(appointment.date.toLocaleDateString("en-GB"));
  const [appointmentOldTime, setAppointmentOldTime] = useState(appointment.startHour);
  const [appointmentTime, setAppointmentTime] = useState(appointment.startHour);
  const [appointmentDate, setAppointmentDate] = useState(appointment.date.toLocaleDateString("en-GB"));
  const [appointmentText, setAppointmentText] = useState(appointment.observations);
  const [appointmentEndTime, setAppointmentEndTime] = useState(appointment.endHour);
  const [appointmentDateDB, setAppointmentDateDB] = useState(appointment.date);
  const [modifyAppointment, setModifyAppointment] = useState(appointment.appointmentId)
  console.log(appointmentDateDB)
  ...
  const handleAppointment = async () => {
    ...
    const getOtherAppointments = new Promise((resolve)=>{
      ...
      let oldDate = appointmentOldDate;
      let OldTime = appointmentOldTime;
      let newDoc = PassId
      ...
      .then((querySnapshot) => {
          querySnapshot.forEach((doc) => {
            ...
            if (otherDate.toDate().toLocaleDateString("en-GB") == newDate){
              if(newDate != oldDate || newStartHour != OldTime){
            count++;}
        }
   ...
   dbRef.doc(modifyAppointment).update({observations: appointmentText,date: appointmentDateDB, startHour: appointmentTime, endHour: appointmentEndTime}).then((res) => {
      setAppointmentDate ("");
      setAppointmentText("");
      setAppointmentTime("");
    ...
    <View style={{ flex: 1 }}>
      <LinearGradient style={styles.container} colors={["#3867B4", "#0F94B4"]}>
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Modificar una cita</Text>
        </View>
      </LinearGradient>
      <ScrollView>
        <View style={styles.body}>
          ...
          <View>
            <TouchableHighlight
              ...
              style={styles.modalButton}
            >
              <Text style={styles.selectText}>Modificar cita</Text>
            </TouchableHighlight>
          </View>
        </View>
      </ScrollView>
